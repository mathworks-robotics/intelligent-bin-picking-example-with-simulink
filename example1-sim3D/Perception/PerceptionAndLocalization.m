classdef (StrictDefaults) PerceptionAndLocalization < matlab.System
    ...
    properties (Nontunable)
        ...
        % Canonical model of the objects
        models =   containers.Map;
        % Canonical model of the objects
        modelPartial =   containers.Map;
    end
    ...
    methods
        % Constructor
        function obj = PerceptionAndLocalization(varargin)
    ...
    methods (Access = protected)
        ...
        function [status, numObjects, objectPose] = stepImpl(obj,ptCloudXYZ, rgbImage, bboxes,label)
            %Obtain pose of objects to pick using PCA on point cloud
            if ~isempty(bboxes)
                ptCloud = pointCloud(ptCloudXYZ);
                %The color threshold for removing the plane from the point cloud data.                
                colorThreshold = 100;
                %Fit plane and remove the plane 
                [~, ~, nonPlaneIdx] = pcfitplane(ptCloud, obj.PlaneThickness, "MaxNumTrials", 3000);
                colorMeetIdx = rgbImage > colorThreshold;
                colorMeetIdx = colorMeetIdx(:, :, 1) & colorMeetIdx(:, :, 2) & colorMeetIdx(:, :, 3);
                
                col_nonPlaneIdx = intersect(find(colorMeetIdx), nonPlaneIdx);
                [m, n, ~] = size(rgbImage);
                nonPlaneMask = false(m*n, 1);
                nonPlaneMask(col_nonPlaneIdx) = true;

                [xyz,~,ptCloudParameterVector] = findObjectPoses(ptCloud,rgbImage, bboxes,label,obj.models,obj.modelPartial,obj.gridDownsample, nonPlaneMask);
                ...
                if ~isempty(ptCloudParameterVector)
                    ...
                    partGT = zeros(obj.maxVisionObjects, 5);
                    for i=1:length(ptCloudParameterVector)
                        ...
                        partGT(i,5) = double(ObjectTypesID(ptCloudParameterVector{i}.class));
                    end
                end
